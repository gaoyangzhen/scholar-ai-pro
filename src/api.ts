...
// Mock Data
const initialDiffs = [
  { original: "The study shows", modified: "The study demonstrates", type: "enhancement" as const },
  { original: "good results", modified: "significant results", type: "enhancement" as const },
];
...
export const api = {
   ...
   */
  polishText: async (text: string, modelId: string, apiKey?: string, onStream?: (chunk: string) => void): Promise<string> => {
    console.log(`[API] Polishing | Model: ${modelId} | Key Present: ${!!apiKey}`);

    if (USE_MOCK_API) {
      return new Promise((resolve) => {
        const mockResponse = "The model was implemented using the PyTorch framework. During the data preprocessing stage, all missing values were excluded. Ultimately, the results indicated an accuracy of 95%. This demonstrates the robustness of our approach under varied conditions.";
        let i = 0;
   ...
   */
  translate: async (text: string, targetLang: string, modelId: string, apiKey?: string, onStream?: (chunk: string) => void): Promise<string> => {
   ...
   * 添加术语
   */
  addGlossaryTerm: async (term: Omit<GlossaryTerm, 'id' | 'createdAt'>): Promise<GlossaryTerm> => {
    if (USE_MOCK_API) {
      return new Promise((resolve) => {
        setTimeout(() => {
   ...
   * 文档对话
   */
  chatWithDoc: async (file: File | null, question: string, modelId: string, apiKey?: string): Promise<string> => {
     console.log(`[API] ChatDoc | Question: ${question}`);
     if (USE_MOCK_API) {
         return new Promise((resolve) => {
             setTimeout(() => {
                 resolve(`[Mock AI] Based on the document "${file?.name || 'file'}", here is the answer to "${question}": \n\nThe methodology described in Section 3 utilizes a transformer-based architecture...`);
             }, 1000);
         });
     } else {
         const formData = new FormData();
  ...
  getReferences: async (): Promise<any[]> => {
    if (USE_MOCK_API) {
      return new Promise((resolve) => {
        setTimeout(() => {
          resolve([
            { id: '1', filename: 'Nature_Paper_2024.pdf', fileType: 'pdf', uploadDate: '2024-03-15', size: '2.4 MB' },
            { id: '2', filename: 'Research_Notes.md', fileType: 'md', uploadDate: '2024-03-10', size: '15 KB' }
          ]);
        }, 500);
      });
    } else {
