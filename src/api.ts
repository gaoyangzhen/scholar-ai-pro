import { DiffItem, HistoryRecord, GlossaryTerm } from './types';
...
export const api = {
   ...
   * 翻译助手：发送文本进行翻译
   */
  translate: async (text: string, targetLang: string, modelId: string, apiKey?: string): Promise<string> => {
    console.log(`[API] Translating to ${targetLang} | Model: ${modelId}`);

    if (USE_MOCK_API) {
      return new Promise((resolve) => {
        setTimeout(() => {
          resolve(`[${targetLang}] ${text} (Translated by ${modelId})`);
        }, 800);
      });
    } else {
      try {
